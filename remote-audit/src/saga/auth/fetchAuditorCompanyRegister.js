...
const fetchAuditorCompanyRegister = (info) => {
    ...
    formData.append('auditor_company_registration_date', info.auditorCompanyRegistrationDate)
    formData.append('auditor_company_registration_number', info.auditorCompanyRegistrationNumber)
    formData.append('auditor_company_licence_number', info.auditorCompanyLicenceNumber)
    formData.append('auditor_company_licence_registration_date', info.auditorCompanyLicenceRegistrationDate)
    formData.append('auditor_company_licence_licenceOrganisation', info.auditorCompanyLicenceLicenceOrganisation)
    ...
    return fetch(`http://localhost:8000/profiles/create-profile`, requestOptions);
};
...
export function* fetchAuditorCompanyRegisterWatcher() {
    yield takeEvery(FETCH_AUDITOR_COMPANY_REGISTER, fetchAuditorCompanyRegisterWorker);
}